...
export function generateMarkdownReport(aggregation, tiers, stats, options = {}) {
  const { title = 'Photo Analysis Report', theme = '', competitionName = '' } = options;
 ...
 */
export function generateJsonReport(aggregation, tiers, stats) {
  return {
    metadata: {
...
export function generateTierMarkdownReport(smartTiers, stats, options = {}) {
  ...
  report += `|------|-------|------------|-----------|\n`;
  report += `| 🟢 Tier 1 (Strong) | ${smartTiers.summary.tier1_count} | ${Math.round((smartTiers.summary.tier1_count / smartTiers.summary.total) * 100)}% | > ${smartTiers.summary.high_threshold} |\n`;
  report += `| 🟡 Tier 2 (Good) | ${smartTiers.summary.tier2_count} | ${Math.round((smartTiers.summary.tier2_count / smartTiers.summary.total) * 100)}% | > ${smartTiers.summary.medium_threshold} |\n`;
  report += `| 🟠 Tier 3 (Borderline) | ${smartTiers.summary.tier3_count} | ${Math.round((smartTiers.summary.tier3_count / smartTiers.summary.total) * 100)}% | ≤ ${smartTiers.summary.medium_threshold} |\n\n`;
  ...
  if (smartTiers.tier1 && smartTiers.tier1.length > 0) {
    report += `## 🟢 Tier 1: Strong Submissions (${smartTiers.tier1.length} photos)\n\n`;
    ...
    smartTiers.tier1.forEach((photo, idx) => {
      ...
      report += `**Score**: ${photo.score.toFixed(1)}/10 | **Status**: ✅ Ready for submission\n\n`;
    });
  }

  // Tier 2 photos
  if (smartTiers.tier2 && smartTiers.tier2.length > 0) {
    report += `## 🟡 Tier 2: Good Submissions (${smartTiers.tier2.length} photos)\n\n`;
    ...
    smartTiers.tier2.forEach((photo, idx) => {
      const displayName = photo.filename || photo.file || photo.name || 'unknown';
      report += `### ${idx + 1}. ${displayName}\n`;
      report += `**Score**: ${photo.score.toFixed(1)}/10 | **Status**: 👍 Good candidate\n\n`;
    });
  }

  // Tier 3 photos
  if (smartTiers.tier3 && smartTiers.tier3.length > 0) {
    report += `## 🟠 Tier 3: Borderline Submissions (${smartTiers.tier3.length} photos)\n\n`;
    ...
    smartTiers.tier3.forEach((photo, idx) => {
      const displayName = photo.filename || photo.file || photo.name || 'unknown';
      report += `### ${idx + 1}. ${displayName}\n`;
      report += `**Score**: ${photo.score.toFixed(1)}/10 | **Status**: ⚠️ Review recommended\n\n`;
    });
  }
...
export function generateTierCsvReport(smartTiers) {
  ...
  const tierData = [
    { tier: smartTiers.tier1, tierLabel: 'Tier1', displayLabel: '🟢 Strong', rec: 'Submit' },
    { tier: smartTiers.tier2, tierLabel: 'Tier2', displayLabel: '🟡 Good', rec: 'Consider' },
    { tier: smartTiers.tier3, tierLabel: 'Tier3', displayLabel: '🟠 Borderline', rec: 'Review' }
  ];
